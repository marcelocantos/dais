...
func TestGetFromDiscovery(t *testing.T) {
	...
	if got.TaskID() != uuid {
		t.Errorf("ID = %q, want %q", got.TaskID(), uuid)
	}
	if got.TaskName() != "/Users/test/project" {
		t.Errorf("Name = %q, want %q", got.TaskName(), "/Users/test/project")
	}
...
func TestKill(t *testing.T) {
	...
	if s.TaskStatus() != claudia.TaskStatusStopped {
		t.Errorf("expected status %q, got %q", claudia.TaskStatusStopped, s.TaskStatus())
	}
